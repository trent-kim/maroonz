...
const WatchedOrb: React.FC<{
  ...
  return isExperienceEnd ? (
    ...
    >
      <div
        ref={orbRef}
        className={`bg-black text-white font-mono text-sm w-lg h-lg border border-white rounded-full flex justify-center items-center opacity-100 transition-all ease-in-out duration-md`}
      >
        {number}
      </div>
    </div>
    ...
    <div
      className="group rounded-full transition-all ease-in-out duration-sm blur"
      style={{
        position: "absolute",
        left: `${position.x}px`,
        top: `${position.y}px`,
